...
function chaussette(droite: string|boolean, gauche: string|number)
{
    ...
    /* 
        Pour rentrer dans la condition, droite et gauche doivent être de même type.
        TS sait donc que droite et gauche, dans la condition de type string, étant le seul type qu'ils ont en commun.
    */        
}
...
function check(a:Date|HTMLElement)
{
    if(isDate(a))
        console.log(a);
    /*
        si j'indique que ma fonction isDate() retourne un boolean, Typescript ne comprendra pas le rôle du boolean.
        Mais en indiquant que ma fonction retourne "a is Date"
        grâce au mot clef "is", TS comprend que le retour d'un boolean à true indique que le type a est bien une date.
    */        
}
